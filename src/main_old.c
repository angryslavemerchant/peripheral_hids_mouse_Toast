...
static void pairing_process(struct k_work *work)
{
	...
	printk("Press any button to confirm pairing.\n");
}
...
int main(void)
{
	...
	printk("Advertising started - Device should be visible as '%s'\n", DEVICE_NAME);
	printk("Waiting for connection...\n");

	/* Main loop - just keep alive and update battery */
	while (1) {
		k_sleep(K_SECONDS(1));
		...
		/* Heartbeat indicator */
		if (++blink_status % 5 == 0) {
			printk("Alive... (advertising: %s)\n", 
			       is_adv_running ? "yes" : "no");
		}
	}
}